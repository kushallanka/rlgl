...
import { CheckCircle2, XCircle, AlertCircle, MinusCircle, HelpCircle, X } from 'lucide-react';
...
function ResultRow({ result, isSelected, onToggleSelect, onUpdateStatus }: ResultRowProps) {
  ...
  return (
    <ListRow isSelected={isSelected}>
      ...
      {/* ID badge */}
      <span className={`${COL.id} text-[11px] font-mono font-semibold text-violet-600 dark:text-violet-400 bg-violet-100 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20 rounded-md px-2 py-0.5 truncate`}>
        T{result.testCaseId}
      </span>
      ...
      <span className="text-sm text-gray-700 dark:text-white/80 flex-1 truncate min-w-0">
        {result.testCaseName ?? (
          <span className="text-gray-400 dark:text-white/25 italic text-xs">No title</span>
        )}
      </span>
      ...
      <DropdownPortal isOpen={open} triggerRef={btnRef} onClose={() => setOpen(false)} width={168}>
        {STATUSES.map(s => {
          ...
          return (
            <button
              key={s.key}
              onClick={() => { if (!active) onUpdateStatus(s.key); setOpen(false); }}
              ...
              {STATUSES.map(s => {
                ...
                return (
                  <motion.button
                    ...
                    style={{ backgroundColor: `rgba(${parseInt(s.color.slice(1,3),16)},${parseInt(s.color.slice(3,5),16)},${parseInt(s.color.slice(5,7),16)},0.10)` }}
                  >
