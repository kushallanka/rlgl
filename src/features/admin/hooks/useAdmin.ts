...
export function useAdmin(projectId: string | null) {
  ...
  const createTypeMutation = useMutation({
    mutationFn: (data: { name: string; projectId: string }) => adminApi.createType(data),
    onSuccess: () => {
  ...
  const createPriorityMutation = useMutation({
    mutationFn: (data: { name: string; projectId: string }) => adminApi.createPriority(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['config-schema', projectId] });
    },
  });
  ...
  const updateFieldMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<any> }) => adminApi.updateField(projectId!, id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['config-schema', projectId] });
    },
  });
  ...
  const createRoleMutation = useMutation({
    mutationFn: (data: { name: string; permissions: string[]; projectId: string }) => adminApi.createRole(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['config-schema', projectId] });
  ...
  const addUserRoleMutation = useMutation({
    mutationFn: (data: { userId: string; roleId: string; projectId: string }) => adminApi.addUserRole(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-roles', projectId] });
    },
  });
  ...
  const updateUserRoleMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { roleId: string } }) => adminApi.updateUserRole(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-roles', projectId] });
    },
  });
  ...
  const getMutationForTab = useCallback(
    (
      tab: TabType,
    ): {
      ...
      switch (tab) {
        case 'types':
          return { create: createTypeMutation, update: updateTypeMutation, delete: deleteTypeMutation };
        case 'priorities':
          return { create: createPriorityMutation, update: updatePriorityMutation, delete: deletePriorityMutation };
        case 'fields':
          return { create: createFieldMutation, update: updateFieldMutation, delete: deleteFieldMutation };
        case 'roles':
          return { create: createRoleMutation, update: updateRoleMutation, delete: deleteRoleMutation };
        default:
          return null;
      }
    },
