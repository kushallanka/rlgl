...
const RESERVED_SLUGS = new Set([
  'api', 'admin', 'auth', 'www', 'mail', 'support', 'help',
  'status', 'blog', 'docs', 'billing', 'app', 'dashboard',
]);
...
export class OrgService {
  ...
  async updateOrg(
    ...
    if (!membership || !['owner', 'admin'].includes(membership.role)) {
      return { ok: false as const, status: 403, error: 'Only owners and admins can update organization', code: 'ORG_FORBIDDEN' };
    }

    const updated = await this.repo.update(orgId, { ...updates, expectedVersion }, actorId);
    if (!updated) {
      return { ok: false as const, status: 409, error: 'Version conflict — organization was modified by another request', code: 'VERSION_CONFLICT' };
    }
  ...
  async addMember(
    ...
    if (!actorMembership || !['owner', 'admin'].includes(actorMembership.role)) {
      return { ok: false as const, status: 403, error: 'Only owners and admins can add members', code: 'ORG_FORBIDDEN' };
    }
  ...
  async removeMember(
    ...
    const actorMembership = await this.repo.getMemberRole(orgId, actorId);
    if (!actorMembership || !['owner', 'admin'].includes(actorMembership.role)) {
      return { ok: false as const, status: 403, error: 'Only owners and admins can remove members', code: 'ORG_FORBIDDEN' };
    }
    ...
    if (targetUserId === actorId && actorMembership.role === 'owner') {
      ...
      if (owners.length <= 1) {
        return { ok: false as const, status: 400, error: 'Cannot remove the last owner of an organization', code: 'LAST_OWNER' };
      }
    }
  ...
  async getAuditLog(orgId: number, actorId: number, page: number, limit: number) {
    const membership = await this.repo.getMemberRole(orgId, actorId);
    if (!membership || !['owner', 'admin'].includes(membership.role)) {
      return { ok: false as const, status: 403, error: 'Only owners and admins can view audit logs', code: 'ORG_FORBIDDEN' };
    }
